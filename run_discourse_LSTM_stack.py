# -*- coding: utf-8 -*-
"""
Created on Mon May 14 23:10:27 2018

@author: Samuel Mensah
"""

from keras.layers import Input, Embedding, Dense, merge, Dropout, Reshape, Lambda, \
    Activation,Flatten,Convolution1D, GlobalMaxPooling1D, TimeDistributed, \
    GlobalAveragePooling1D, LSTM, Highway, BatchNormalization
...
" Implicit and Explicit discourse relation using LSTM"
...
def cnn_model(data, X_train, weights=None, name='augmented'):
    ...
    embeddings = Embedding(input_dim = word_WE.shape[0], input_length = X_train.shape[1], weights=[word_WE],
                             output_dim= word_WE.shape[1], trainable=False, 
                            mask_zero=False, dropout=0.1)(main_input)
    
    # Propagate the embeddings through an LSTM layer with 128-dimensional hidden state
    # return_seq = True, to return history of hidden state at each time step for LSTM stacks
