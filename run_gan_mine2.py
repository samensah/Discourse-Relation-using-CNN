...
from keras.layers import Input, Embedding, Dense, concatenate, merge, Dropout, Reshape, Lambda, \
    Activation, Convolution1D, GlobalMaxPooling1D, GlobalAveragePooling1D, LSTM, Highway, BatchNormalization
...
class TrainModel:
    def __init__(self, word_WE, params, num_class):
        ...
        self.filter_num = 256
        self.filter_lengths = [2, 5, 10]
        self.cnn_dense_num = 0
        self.cnn_dense_size = 512
        self.cnn_avgpool = True  # average pooling or max pooling (only for cnn1)
        ...
        self.discr_dense_num = 4
        self.discr_dense_size = 1024
        ...
        self.best_rec_epochs = 10  # only record after 100 epochs
        ...
        self._embed_word = Embedding(input_dim=word_WE.shape[0], input_length=self.arg_maxlen, weights=[word_WE],
                                     output_dim=word_WE.shape[1], trainable=False, mask_zero=False)
    ...
    def _build_cnn(self, filter_num, filter_lengths, cnn_dense_num, cnn_dense_size, cnn_kmax, cnn_avgpool):
        ...
        arg1_cnns = [Convolution1D(filters=filter_num, kernel_size=i,
                                   padding='same', activation=self.activation) for i in filter_lengths]
        arg2_cnns = arg1_cnns
        ...
        arg1_cnn_merge = concatenate(arg1_cnn_outpus, axis=-1)
        arg2_cnn_merge = concatenate(arg2_cnn_outpus, axis=-1)
        pooling_part = GlobalMaxPooling1D()  # for i-CNN
        if cnn_kmax > 1:
            pooling_part = GlovalKMaxAveragePooling1D(cnn_kmax)
        if cnn_avgpool:
            pooling_part = GlobalAveragePooling1D()  # for a-CNN
    ...
    def _build_discr(self):
        ...
        model = Model(inputs=inps, outputs=[predictions, d_feature, aux_pred])
        return model
    ...
    def _build_cnn_discr(self, block_cnn_ori, block_cnn_gen, block_cnn_discr, block_discr, lr):
        ...
        model.compile(loss=[binary_ce, binary_ce, multi_ce, multi_ce],
                      optimizer=self.get_opt(self.D_optimizer_name)(lr))
        return model

    # train cnn_origin+ori_clf+(freeze)discr
    def _build_cnn_ori_clf_freezediscr(self, block_cnn_ori, block_clf_ori, block_cnn_gen, block_cnn_discr, block_discr,
                                       lr):
        ...
        model.compile(loss=[multi_crossentropy1, binary_crossentropy2, fm_lfunc, dfm_lfunc],
                      optimizer=self.get_opt(self.cnn_optimizer_name)(lr))
        return model

    # train cnn_joint+clf+(freeze)discr
    def _build_cnn_joint_clf_freezediscr(self, block_cnn_ori, block_clf_ori, block_cnn_gen, block_clf_gen,
                                         block_cnn_discr, block_discr, lr):
        ''' For cnn_origin and model_classifier training
    ...
    def build_all_model(self):
        ...
        lrs = self.lrs
        # basic blocks
        blocks['cnn_ori'] = self._build_cnn(self.filter_num, self.filter_lengths, self.cnn_dense_num,
                                            self.cnn_dense_size, self.kmax0, False)
        blocks['cnn_gen'] = blocks['cnn_ori']
        if self.cnn01_diff:
            blocks['cnn_gen'] = self._build_cnn(self.filter_num, self.filter_lengths, self.cnn_dense_num,
                                                self.cnn_dense_size, self.kmax, self.cnn_avgpool)
        if self.discr_filter_num:
        ...
        models['joint+clf'] = self._build_cnn_joint_trainer(blocks['cnn_ori'], blocks['cnn_gen'], blocks['clf_ori'],
                                                            blocks['clf_gen'], lrs['joint+clf'])
        models['discr'] = self._build_cnn_discr(blocks['cnn_ori'], blocks['cnn_gen'], blocks['cnn_discr'],
                                                blocks['discr'], lrs['discr'])
        models['ori+clf+discr'] = self._build_cnn_ori_clf_freezediscr(blocks['cnn_ori'], blocks['clf_ori'],
                                                                      blocks['cnn_gen'], blocks['cnn_discr'],
                                                                      blocks['discr'], lrs['ori+clf+discr'])
    ...
    def _prepare_inputs(self, data_batched, add_arg2=1, add_arg2plus=0, drop_arg2plus=0., add_y=0):
        def _prepare_arg2plus(drop):
            ...
            ret = np.asarray(
                [(data_batched['arg2'][i] if (np.random.random_sample() < drop) else data_batched['arg2plus'][i]) for i
                 in range(size)])
            return ret
    ...
    def _fit_one(self, model_name, data_batched):
        ...
        if model_name == 'ori+clf':
            ...
            loss = m.train_on_batch(
                self._prepare_inputs(data, add_arg2=0, add_arg2plus=1, drop_arg2plus=self.drop_conn), [data['sense']])
        elif model_name == 'joint+clf':
            loss = m.train_on_batch(self._prepare_inputs(data, add_arg2plus=1, drop_arg2plus=self.drop_conn),
            ...
            loss = m.train_on_batch(self._prepare_inputs(data, add_arg2plus=1, add_y=self.discr_ydim),
                                    [y_0, y_1, data['sense'], data['sense']])
        elif model_name == 'ori+clf+discr':
            y_1 = np.asarray([self.alpha for i in data['arg1']])
            loss = m.train_on_batch(self._prepare_inputs(data, add_arg2plus=1, add_y=self.discr_ydim),
                                    [data['sense'], y_1, y_1, y_1])
        else:
        ...
        # Phase 2, train discr for m epochs
            ...
            for data in self._generate_batch(train_data, self.batch_size, self.shuffle, True):
                ...
                for i in range(self.kd):
                    phase = self._test_discr(
                        train_data if self.thresh_by_whole else datas[np.random.randint(0, len(datas))])
    ...
    def fit(self, train_data, dev_data, test_data, save_best=True):
        # use whole epoch for one model
        best_acc = {}
        best_epoch = {}
        history = {}
        data_dt = {'dev': dev_data, 'test': test_data}
        for n in ['dev', 'test']:
            best_acc[n] = 0.
            best_epoch[n] = -1
            history[n] = {}
        ...
        for epoch in range(self.epoch):
            ...
            Oracle.print()
        result_ori = self._test_one('ori+clf', data_dt['test'])
...
def run_one(data, selections, indexes, output_prefix):
    ...
    with tf.Session(config=config) as s:
        ...
        with tf.device("/gpu:0"):
            ...
            results += model.fit(data['train_data'], data['dev_data'], data['test_data'])
    ...
    with open(output_prefix + '.csv', 'a+') as csvfile:
        try:
            ...
            writer.writerow([results[2]['dev'], results[1]['dev']] + [(str(k) + ":" + str(selected_param[k])) for k in
                                                                      sorted(selected_param.keys())] + [title])
        except:
...
s1adamv3 = [
    {'strategy': (3,), 'lambda_classify': (1.0,), 'lambda_confuse_binary': (0.1, 0.05),
     'lambda_confuse_fm': (0., 0.05, 0.1),
...
stra4 = [
    {'strategy': (3,), 'lambda_classify': (1.0,), 'lambda_confuse_binary': (0.1, 0.05),
     'lambda_confuse_fm': (0., 0.05, 0.1),
     'lambda_gen': (0.5,), 'drop_conn': (0.1, 0.3, 0.5), 'epoch_firstjoint': (3,), 'thresh_by_whole': (False,),
     'thresh_high': (0.99, 0.95, 0.9), 'thresh_by_acc': (True,), 'kd': (5, 3, 1), 'shuffle': (3,),
     'filter_num': (256,), 'filter_lengths': ([2, 5, 10],), 'filter_diff': (True,),
     'dense_num': (1,), 'dense_size': (512,), 'dense_dropout': (0.3,),
     'lrs': ({'ori+clf+discr': 0.001, 'joint+clf': 0.001, 'discr': 0.0001},), 'D_optimizer_name': ("Adam"),
     'discr_dense_num': (4, ), 'discr_dense_size': (1024, ), 'discr_dense_dropout': (0.1,), 'discr_ydim': (False,),
     'cnn_avgpool': (True, ), 'best_rec_epochs': (10,), 'verbose': (False,), 'batch_size': (128,)},
    "stra4",
    -1
]
...
def main(method_name):
    ...
    run_dict, run_name, run_max_time = method_name
    indexes = []
    ...
    data = None
    for fn in ["data_f0-r0.5-w36128-p45.pic"]:
        try:
    ...
    run_indexes = [int(i) for i in indexes]
