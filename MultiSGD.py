...
def set_lr_mult(model):
    # setup lr multiplier for embedding layer
    lr_mult = dict()
    for layer in model.layers:
        #choose the embedding layer for update
        if layer.weights != []:
            for weight in layer.weights:
                if re.match("embedding_1/*", weight.name):
                    embed_name = weight.name
                    lr_mult[embed_name] = 2
